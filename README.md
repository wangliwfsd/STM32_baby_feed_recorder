...
- 可调奶量按钮默认 `150 ml`，左右滑动以 `10 ml` 为步长调整
- 实时显示当前 RTC 时间
- 显示当天总奶量、次数和最近一次记录
- 最新记录显示在列表顶部
- 上下滑动浏览今日及历史记录
...
注意：开发板没有独立 RTC 备用电池，`VBAT` 与主电源相关。完全断电后 RTC 时间不能可靠保留；RTC 失效时，时间设置页默认显示 `2026-07-13 00:00:00`，也可通过联网 SNTP 重新校时。
...
- 待保存记录数 `P:n`
- 六个固定奶量快捷按钮
- 一个可调奶量按钮：短按记录，右滑增加 10 ml，左滑减少 10 ml
- 单条记录删除按钮
...
FreeRTOS heap 配置为 64 KB，为 SD 存储任务、LwIP TCP/IP 线程、Ethernet 接收线程和 DHCP 邮箱保留足够的动态内存。网络初始化会检查接口、线程和信号量创建结果；没有取得 IP 时再次同步会重新启动 DHCP，网线重新插入后也会重新申请地址。
...
- 尝试更换路由器端口或网线
- 检查局域网是否限制未知 MAC 地址
- 如果菜单显示 `NET: INIT ERROR`，表示网络线程、信号量或底层 Ethernet 初始化失败，而不是普通 DHCP 超时
